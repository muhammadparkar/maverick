...
export default function Testimonials() {
  return (
    <section className="border-t border-white/10 bg-coffee py-20 sm:py-28">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {testimonials.map((t, i) => (
            <Reveal key={t.name} delay={i * 90}>
              <figure className="flex h-full flex-col rounded-2xl border border-white/10 bg-coffee-800 p-7">
                <Stars />
                <blockquote className="mt-4 flex-1 text-sm leading-relaxed text-crema">
                  “{t.quote}”
                </blockquote>
                <figcaption className="mt-6 flex items-center gap-3 border-t border-white/10 pt-5">
                  ...
                  />
                  <div>
                    <p className="font-display text-sm font-bold uppercase tracking-wide text-crema">
                      {t.name}
                    </p>
                    <p className="text-xs text-crema-dim">{t.role}</p>
                  </div>
                </figcaption>
              </figure>
            </Reveal>
