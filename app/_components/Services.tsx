...
export default function Services() {
  return (
    <section className="relative overflow-hidden border-y border-white/10 bg-coffee-800 py-20 sm:py-28">
      {/* roaster backdrop on the right */}
      <div className="pointer-events-none absolute inset-y-0 right-0 hidden w-1/2 lg:block">
        <Image src="/bg-roaster.jpg" alt="" fill sizes="50vw" className="object-cover opacity-20" />
        <div className="absolute inset-0 bg-gradient-to-r from-coffee-800 to-transparent" />
      </div>
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2">
          {services.map((s, i) => (
            <Reveal key={s.title} delay={i * 80}>
              <article className="flex h-full gap-4 rounded-2xl border border-white/10 bg-coffee/70 p-6 backdrop-blur-sm transition-colors hover:border-copper/40">
                <span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-copper/40 bg-copper/10 text-copper-bright">
